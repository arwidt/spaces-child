...
<div class="row fadein portfolio<?php if ($full == true) { echo ' fullscreen';} ?>">

	<?php if ( $masonry == true ) {  ?>
		<ul id="masonry-container" class="grid <?php if ($full == true) { echo ' fullscreen';} ?> <?php echo get_theme_mod( 'portfolio_column_width');?>">
	<?php } else { ?>
		<ul id="portfolio-grid" class="grid hfeed<?php if ($full == true) { echo ' fullscreen';} ?> <?php echo get_theme_mod( 'portfolio_column_width');?>">
	<?php }
		...
		{ 	
			if ( ( function_exists('has_post_thumbnail')) && (has_post_thumbnail()) ) { ?>
				     ...
				     <li id="portfolio-<?php the_ID(); ?>" class="item masonry-item grid-masonry type-portfolio <?php echo $thumbnail; ?> <?php echo $term_list;?>">
				          
				          <div id="post-<?php the_ID(); ?>" <?php post_class('portfolio'); ?>>

				          	<?php if ( $lightbox == true ) { 
								bean_gallery($post->ID, '', 'port-masonry-lightbox' , 'post__in' , true); 
							} else { ?>
		...
		if ( $masonry != true ) 
		{
			...
			{ 
				...
				<li id="load-more" class="item masonry-item grid-masonry">
					<div class="portfolio">
						<a href="#" class="entry-link" id="load-posts" data-posts-container="<?php echo $portfolio_items_container; ?>" data-load-more-action="bean_load_more_portfolio" data-loop-template="<?php echo $loop_template; ?>" data-order="<?php echo $order; ?>" data-orderby="<?php echo $orderby; ?>" data-metakey="<?php echo $meta_key; ?>" data-posts-count="<?php echo $portfolio_posts_count; ?>">
							<span class="loading">
								<div class="spinner">
						...
						<div class="overlay">
							<h5 class="load-more"><?php echo '<span>' . __('Load More', 'bean') . '</span>'; ?></h5>
							<h5 class="load-finished"><?php echo '<span>' . __('Done Loading', 'bean') . '</span>'; ?></h5>
						</div>
					</div>
				</li> 
